...
import { collection, getDocs, query, orderBy, limit, startAfter, endBefore, where , doc, updateDoc } from "firebase/firestore";
...
const PublisherApprovalTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const countQuery = query(collection(db, "users"), where("approved", "==", true));
  ...
  const handleActionClick = async (id: string, action: boolean) => {
    try {
      ...
      await updateDoc(userDoc, { approved: action });

      // Trigger the animation by adding a class
      setPublishers(prev => prev.map(publisher =>
        publisher.id === id ? { ...publisher, isRemoving: true } : publisher
      ));

      // After animation ends, remove the publisher
      setTimeout(() => {
        setPublishers(prev => prev.filter(publisher => publisher.id !== id));
      }, 300);  // Duration of the Tailwind transition
    } catch (error) {
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {loading ? (
            ...
            publishers .filter(publisher => publisher.role !== 'admin').map((publisher, index) => (
              <tr key={publisher.id} className={`transition-opacity duration-300 ${publisher.isRemoving ? 'opacity-0' : 'opacity-100'}`}>
                ...
                <td className="px-6 py-4">{publisher.role}</td>
